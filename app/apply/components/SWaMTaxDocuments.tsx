...
interface SWaMTaxDocumentsProps {
...
export function SWaMTaxDocuments({
register,
watch,
control,
handleInputFocus,
handleInputBlur
}: SWaMTaxDocumentsProps) {
...
const documents = [
  { name: "Most recent year tax return - COMPLETE", info: "Upload the full tax return for the most recent year" },
  { name: "Tax return 2 years ago - FIRST PAGE ONLY -OR- 1040 1st pg AND Schedule C if on 1040", info: "Upload only the first page of the tax return from 2 years ago, or the first page of 1040 and Schedule C if applicable" },
  { name: "Tax return 3 years ago - FIRST PAGE ONLY -OR- 1040 1st pg AND Schedule C if on 1040", info: "Upload only the first page of the tax return from 3 years ago, or the first page of 1040 and Schedule C if applicable" },
]
...
const handleFile = async (file: File, documentName: string) => {
  ...
  try {
    ...
    const filePath = `swam-tax-documents/${documentName}/${fileName}`
...
return (
  <Card className="w-full max-w-4xl mx-auto">
    <CardHeader>
      <CardTitle className="text-2xl font-bold">SWaM Tax Documents</CardTitle>
    </CardHeader>
    <CardContent>
          ...
          <div key={index} className="space-y-2">
            ...
            {!fileUploads[doc.name] ? (
              <div
                ...
                onDragEnter={(e) => handleDrag(e, doc.name)}
                onDragLeave={(e) => handleDrag(e, doc.name)}
                    ...
                    >
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (file) handleFile(file, doc.name)
                        }}
                        onFocus={() => handleInputFocus(`swamTaxDocuments.${index}`)}
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          {formatFileSize(fileUploads[doc.name].size)}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center space-x-2">
                      {fileUploads[doc.name].status === 'uploading' && (
                        <div className="w-24">
                          <Progress value={fileUploads[doc.name].progress} />
                        </div>
                      )}
                      <Button
                        variant="ghost"
                  ...
                  {fileUploads[doc.name].preview && (
                    <div className="mt-2">
                      <img
                        src={fileUploads[doc.name].preview}
                        alt="Preview"
